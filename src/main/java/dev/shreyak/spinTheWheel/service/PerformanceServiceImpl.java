...
//                    summary.append("No matches found in database where " + batsman + " played against " + bowler + "\n");
    ...
    public String getSummaryForPlayers(String players[], String venue, String type, String afterDate) {
        Map<String, StringBuilder> venueSummaries = new HashMap<>();
        ...
        for (String player : players) {
            ...
            for (Match match : matches) {
                ...
                if (hasBowled) {
                    if(!temp.isBlank()) temp = temp + "\n" + match.bowlerSummary(player, match);
                    else temp = match.bowlerSummary(player, match);
                }

                if (temp.isBlank()) continue;

                String stadium = match.getInfo().getVenue();
                venueSummaries.putIfAbsent(stadium, new StringBuilder());
                venueSummaries.get(stadium).append(temp);
                venueSummaries.get(stadium).append("\n");
            }
        }

        StringBuilder summary = new StringBuilder();
        for (Map.Entry<String, StringBuilder> entry : venueSummaries.entrySet()) {
